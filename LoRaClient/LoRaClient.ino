...
int led = 8;           // Pro Micro P19, A1, D14
int nsel = 10;          // Pro Micro P18, A0,   SX1278  CS
int sck = 11;
int mosi = 13;
int miso = 12;
int dio0 = 7;
...
{
  unsigned char addr;
  Serial.println("Serial: Enter sx76 Config");
...
unsigned char sx1278_LoRaRxPacket (void)
{
  ...
  if (digitalRead(dio0))  {
         Serial.println("Serial: DIO_0 shows packet recieved");
...
void setup() {
    ...
    Serial.println("########################################");
    delay(10);  
    Serial.println("Serial: Software Serial Port Connected");
    delay(10);  
    Serial.println("#########################################");
    for (int i=0; i<200; i++);
    {
...
// this is the main code to run repeatedly
  ...
  Fsk_Rate_Sel  = 0x00;
        ...
        Serial.println("Serial: RX Data buffer contains");
        for (int i=0; i<64; i++) {
  ...
  // Slave
  Serial.println("Serial: Waiting for RX packet  ");
        char fromSPI = SPIRead(LR_RegFifoRxCurrentAddr);
        Serial.print("Serial: Reg FIFO Rx Current Address  ");
        ...
        Serial.println();
        Serial.print("Serial: RegModem (LoRa) Stat 0x");
       ...
       if (showOpMode && 0x80 == 0 )
           Serial.println("Serial: OpMode is FSK");
       else
           Serial.println("Serial: OpMode is LoRa");
        
       showOpMode = SPIRead(LR_RegOpMode);// && 0xF8;
       Serial.print("Serial: OpMode Regsiters = ");
       Serial.print(showOpMode, HEX);
       Serial.println(" ");
       
           
       char RSSIVal = SPIRead(0x1B);
       Serial.print("Serial: RSSI is ");
